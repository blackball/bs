...
/*
  ...
  frame = getFirstFrame(videoStream);
  if (!bg.ModelInit(frame->imageData), frame->width, frame->height, frame->nChannels)
  {
  fprintf(stderr, "Error: Could not Initialize correctly!\n");
  return -1;
  }
  // one channel w*h segmentation map. foreground 0xFF, else 0x00
  seg_map = (unsigned char*)malloc(sizeof(unsigned char) * frame->width * frame->height);

  while ( frame = getNextFrame(videoStream) )
  {
  bg.ModelUpdate(frame->imageData, seg_map);
...
BGooth(
  ...
  int ModelInit(const unsigned char *imageData,
    ...
    switch(nChannels) {
      case 1:
        /* gray scale */
        return ModelInit_C1R(imageData, imgW, imgH);
      case 3:
        /* three channels color image */
        return ModelInit_C3R(imageData, imgW, imgH);
...
#define _at(x,y) (*(imgPtr + y*width+ x))
...
#undef _at
  ...
  inline void getRandomNeighborPixels_C3R(const unsigned char *imageData,
    ...
    const unsigned char *imgPtr = imageData + 3*(h * width + w);
...
#define _at(x,y,i) (*(imgPtr + 3*(width*y+x) + i))
